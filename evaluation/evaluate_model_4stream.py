"""
tests the model on doctamper testing set
"""
...
resize_to = 512
raw_predictions_path = 'results/raw_predictions_4stream.npz'
batch_size = 8
...
if os.path.exists(raw_predictions_path):
    ...
    print(f"Evaluating on {len(keep_indices)} test images, batch size {batch_size}, single pass, no interim saves...")
...
with open('results/evaluation_docTamper_4stream.txt', 'w') as f:
    f.write(f"Model: 4-stream (RGB+ELA+DCT+Noise), gated fusion, Lovász loss\n")
    f.write(f"Checkpoint epoch: 9 (of 10 planned; final epoch not completed due to GPU quota constraints)\n")
...
print("\nResults saved to results/evaluation_docTamper_4stream.txt")